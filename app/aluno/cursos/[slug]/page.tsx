...
import { api, type EnrolledCourseResponse } from "@/lib/api";
...
export default function CourseDetailsPage() {
  ...
  useEffect(() => {
    async function loadCourse() {
      ...
      try {
        // Primeiro buscamos o curso pelo slug
        // para descobrir o ID do curso.
        const publicCourse =
          await api.getCourseBySlug(slug);

        // Depois buscamos o curso através do endpoint
        // protegido. Isso garante que o usuário
        // realmente esteja matriculado.
      ...
      <main className="min-h-screen bg-[var(--background)] text-[var(--foreground)]">
        <div className="mx-auto flex min-h-screen max-w-[1440px] items-center justify-center px-6">
          <div className="text-center">
            <div className="mx-auto mb-5 h-10 w-10 animate-spin rounded-full border-2 border-white/10 border-t-[var(--accent)]" />
  ...
  if (error || !course) {
    return (
      <main className="min-h-screen bg-[var(--background)] text-[var(--foreground)]">
        <div className="mx-auto flex min-h-screen max-w-[1440px] items-center justify-center px-6">
          <div className="w-full max-w-lg rounded-2xl border border-white/10 bg-[var(--surface)] p-8 text-center">
            <div className="mx-auto mb-5 flex h-14 w-14 items-center justify-center rounded-full border border-red-500/20 bg-red-500/5">
              <BookOpen
                size={24}
            ...
            <p className="mt-3 text-sm leading-relaxed text-[var(--muted)]">
              {error ||
                "Você não está matriculado neste curso ou o curso não está disponível."}
            </p>
